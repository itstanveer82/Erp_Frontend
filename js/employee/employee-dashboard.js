document.addEventListener("DOMContentLoaded", function () {
  ...
  const employeeProfileButton = document.getElementById(
    "employeeProfileButton",
  );

  const employeeProfileDropdown = document.getElementById(
    "employeeProfileDropdown",
  );

  const headerEmployeeImage = document.getElementById("headerEmployeeImage");

  const headerEmployeeName = document.getElementById("headerEmployeeName");

  const headerEmployeeId = document.getElementById("headerEmployeeId");
  ...
  function loadHeaderEmployeeProfile() {
    fetchCurrentUserProfile()
      .then(function (res) {
        ...
        const employeeImage = p.profileImage;

        headerEmployeeName.textContent = fullName || "Employee";

        headerEmployeeId.textContent = p.employeeCode || "EMP-0000";

        headerEmployeeImage.src = employeeImage;

        profileDropdownName.textContent = fullName || "Employee";

        profileDropdownId.textContent = p.employeeCode || "EMP-0000";

        profileDropdownImage.src = employeeImage;
      })
      .catch(function (error) {
        console.error("Employee header profile error:", error);
      });
  }
  ...
  function loadDashboardSummary() {
    ...
    fetchCurrentUserProfile().then(function (res) {
      ...
      safeSetText("userInitials", initials || "E");
    });
    function safeSetText(elementId, text) {
      ...
      if (el) {
        ...
        console.warn(
          `Element #${elementId} not found — skipping textContent update.`,
        );
      }
    }
    ...
    demoGetDashboardSummary().then(function (res) {
      ...
      document.getElementById("statAttendance").textContent =
        s.attendancePercent + "%";
      document.getElementById("statLeaveBalance").textContent =
        s.leaveBalanceDays + " days";
      document.getElementById("statCorrections").textContent =
        s.pendingCorrections;
      document.getElementById("statLastPayslip").textContent =
        s.lastPayslipMonth;
    });
  }
  ...
  document
    ...
    .addEventListener("change", function () {
      ...
      const filtered =
        value === "ALL"
          ? myAttendanceData
          : myAttendanceData.filter(function (r) {
  ...
  function loadApplyLeaveForm() {
    demoGetLeavePolicy().then(function (res) {
      document.getElementById("leavePolicyLeft").innerHTML = res.data.left
        .map(function (item) {
          return `<li>${escapeHtml(item)}</li>`;
        })
        .join("");

      document.getElementById("leavePolicyRight").innerHTML = res.data.right
        .map(function (item) {
          return `<li>${escapeHtml(item)}</li>`;
        })
        .join("");
    ...
    demoGetLeaveFormMeta().then(function (res) {
      ...
      document.getElementById("ccSelect").innerHTML =
  ...
  function calculateLeaveDays() {
    ...
    daysField.value =
      diff > 0 ? diff + (diff === 1 ? " day" : " days") : "Invalid range";
  }

  document
    .getElementById("leaveFromDate")
    .addEventListener("change", calculateLeaveDays);
  document
    .getElementById("leaveToDate")
    .addEventListener("change", calculateLeaveDays);
  ...
  document.getElementById("leaveType").addEventListener("change", function () {
    ...
    demoGetLeaveBalance().then(function (res) {
      ...
      balanceField.value = match
        ? `${match.remainingDays} / ${match.totalDays} days`
        : "N/A";
    });
  });
  //=============================
  ...
  function renderCcChips() {
    ...
    container.innerHTML = ccSelectedList
      .map(function (c) {
        return `
            <span class="cc-chip">
                ${escapeHtml(c.name)}
  ...
  function loadPayroll() {
    ...
    demoGetPayrollHistory().then(function (res) {
      ...
      tbody
        .querySelectorAll("[data-payroll-download-index]")
        .forEach(function (btn) {
          btn.addEventListener("click", function () {
  ...
  const correctionFormMessage = document.getElementById(
    "correctionFormMessage",
  );
  ...
  function loadProfile() {
    ...
    fetchCurrentUserProfile().then(function (res) {
      ...
      const resetPasswordToggleButton = document.getElementById(
        "resetPasswordToggleButton",
      );
      ...
      const viewFullProfileButton = document.getElementById(
      ...
      const profileAvatarClickable = document.getElementById(
  ...
  function openUploadPhotoModal() {
    ...
    const previewFallback = document.getElementById(
      "uploadPhotoPreviewFallback",
    );
  ...
  const openUploadPhotoButton = document.getElementById(
    "openUploadPhotoButton",
  );
  ...
  if (savePhotoButton) {
    savePhotoButton.addEventListener("click", function () {
      ...
      reader.onload = function (e) {
        ...
        uploadProfilePhoto(selectedPhotoFile)
          .then(function () {
          ...
          .catch(function (error) {
            ...
            messageBox.innerHTML = `<div class="custom-alert error">${escapeHtml(error.message || "Upload failed.")}</div>`;
  ...
  function loadProfileOverview() {
    ...
    Promise.all([
      ...
      grid.innerHTML = `
                ...
                <div class="overview-card">
                    ...
                    <div class="overview-card-body">
                        ${
                          skills.length
                            ? `<div class="cc-chips">${skills
                                .map(function (s) {
                                  return `<span class="cc-chip">${escapeHtml(s)}</span>`;
                                })
                                .join("")}</div>`
      ...
      const overviewEditPersonalInfo = document.getElementById(
        "overviewEditPersonalInfo",
      );
      if (overviewEditPersonalInfo) {
        overviewEditPersonalInfo.addEventListener(
          "click",
          openPersonalInfoModal,
        );
      }

      const overviewEditJoiningDetails = document.getElementById(
        "overviewEditJoiningDetails",
      );
      if (overviewEditJoiningDetails) {
        overviewEditJoiningDetails.addEventListener(
          "click",
          openJoiningDetailsModal,
        );
      }
  ...
  if (resetPasswordForm) {
    resetPasswordForm.addEventListener("submit", function (e) {
      ...
      if (newPassword !== confirmPassword) {
        resetPasswordMessage.innerHTML = `<div class="custom-alert error">New password and confirm password do not match.</div>`;
        return;
      }

      if (newPassword.length < 6) {
        resetPasswordMessage.innerHTML = `<div class="custom-alert error">Password must be at least 6 characters.</div>`;
        return;
      }
      ...
      changeEmployeePassword(currentPassword, newPassword)
        ...
        .catch(function (error) {
          resetPasswordMessage.innerHTML = `<div class="custom-alert error">${escapeHtml(error.message)}</div>`;
        });
    });
  }
  ...
  const newPasswordFieldForLiveCheck =
    document.getElementById("newPasswordField");
  const confirmPasswordFieldForLiveCheck = document.getElementById(
    "confirmPasswordField",
  );

  if (newPasswordFieldForLiveCheck && confirmPasswordFieldForLiveCheck) {
    newPasswordFieldForLiveCheck.addEventListener(
      "input",
      checkPasswordMatchLive,
    );
    confirmPasswordFieldForLiveCheck.addEventListener(
      "input",
      checkPasswordMatchLive,
    );
  }
  ...
  function openJoiningDetailsModal() {
    ...
    document.getElementById("dateOfJoiningField").value =
      currentJoiningDetails.dateOfJoining || "";
    document.getElementById("confirmationDateField").value =
      currentJoiningDetails.confirmationDate || "";
    document.getElementById("joiningStatusField").value =
      currentJoiningDetails.status || "Active";
    document.getElementById("joiningDetailsFormMessage").innerHTML = "";

    const modal = new bootstrap.Modal(
      document.getElementById("joiningDetailsModal"),
    );
    modal.show();
  }

  const editJoiningDetailsButton = document.getElementById(
    "editJoiningDetailsButton",
  );
  ...
  if (joiningDetailsForm) {
    joiningDetailsForm.addEventListener("submit", function (e) {
      ...
      const payload = {
        ...
        confirmationDate: document.getElementById("confirmationDateField")
          .value,
  ...
  function renderNominationTable() {
    ...
    tbody.innerHTML = nominationData
      .map(function (n) {
        return `
                <tr>
  ...
  function openNominationModal(id) {
    ...
    if (id) {
      ...
      document.getElementById("nominationFamilyMember").value =
        record.familyMember;
    ...
    const modal = new bootstrap.Modal(
      document.getElementById("nominationModal"),
    );
    modal.show();
  }
    ...
    nominationForm.addEventListener("submit", function (e) {
      ...
      const payload = {
        ...
        familyMember: document
          .getElementById("nominationFamilyMember")
          .value.trim(),
        percentage: Number(
          document.getElementById("nominationPercentage").value,
        ),
      };
  ...
  function openPersonalInfoModal() {
    ...
    document.getElementById("heightField").value =
      currentPersonalInfo.height || "";
    document.getElementById("weightField").value =
      currentPersonalInfo.weight || "";
    document.getElementById("passportNoField").value =
      currentPersonalInfo.passportNo || "";
    document.getElementById("panNoField").value =
      currentPersonalInfo.panNo || "";
    document.getElementById("aadharNoField").value =
      currentPersonalInfo.aadharNo || "";
    document.getElementById("religionField").value =
      currentPersonalInfo.religion || "";
    document.getElementById("maritalStatusField").value =
      currentPersonalInfo.maritalStatus || "Single";
    document.getElementById("bloodGroupField").value =
      currentPersonalInfo.bloodGroup || "O+";
    document.getElementById("shiftField").value =
      currentPersonalInfo.shift || "Day";
  ...
  const editPersonalInfoButton = document.getElementById(
  ...
  function renderEmergencyContactsTable() {
    ...
    tbody.innerHTML = emergencyContactsData
      .map(function (c) {
        return `
                <tr>
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            demoDeleteEmergencyContact(id).then(function () {
              emergencyContactsData = emergencyContactsData.filter(
                function (c) {
  ...
  function openEmergencyContactModal(id) {
    ...
    const modal = new bootstrap.Modal(
      document.getElementById("emergencyContactModal"),
    );
    modal.show();
  }

  const addEmergencyContactButton = document.getElementById(
    ...
    emergencyContactForm.addEventListener("submit", function (e) {
      ...
      const payload = {
        ...
        relationship: document
          .getElementById("emergencyContactRelationship")
          .value.trim(),
        ...
        address: document
          .getElementById("emergencyContactAddress")
          .value.trim(),
        priority:
          Number(document.getElementById("emergencyContactPriority").value) ||
          null,
      };
  ...
  function renderEducationCards() {
    ...
    container
      .querySelectorAll("[data-delete-education]")
      .forEach(function (btn) {
        btn.addEventListener("click", function () {
    ...
    educationForm.addEventListener("submit", function (e) {
      ...
      const payload = {
        qualification: document
          .getElementById("educationQualification")
          .value.trim(),
        institution: document
          .getElementById("educationInstitution")
          .value.trim(),
  ...
  function renderExperienceTable() {
    ...
    tbody.innerHTML = experienceData
      .map(function (x) {
        return `
                <tr>
  ...
  function openExperienceModal(id) {
    ...
    if (id) {
      ...
      document.getElementById("experienceOrganization").value =
        record.organization;
      document.getElementById("experiencePosition").value = record.position;
      document.getElementById("experienceReasonOfLeaving").value =
        record.reasonOfLeaving;
      document.getElementById("experienceLastCtc").value = record.lastCtc;
      document.getElementById("experienceLastContactNo").value =
        record.lastContactNo;
      document.getElementById("experienceLastReferenceNo").value =
        record.lastReferenceNo;
      document.getElementById("experienceTotalLength").value =
        record.totalLength;
    } else {
    ...
    experienceForm.addEventListener("submit", function (e) {
      ...
      const payload = {
        ...
        organization: document
          .getElementById("experienceOrganization")
          .value.trim(),
        position: document.getElementById("experiencePosition").value.trim(),
        reasonOfLeaving: document
          .getElementById("experienceReasonOfLeaving")
          .value.trim(),
        lastCtc: document.getElementById("experienceLastCtc").value.trim(),
        lastContactNo: document
          .getElementById("experienceLastContactNo")
          .value.trim(),
        lastReferenceNo: document
          .getElementById("experienceLastReferenceNo")
          .value.trim(),
        totalLength: document
          .getElementById("experienceTotalLength")
          .value.trim(),
      };
  ...
  function renderFamilyTable() {
    ...
    tbody.innerHTML = familyMembersData
      .map(function (m) {
        return `
                <tr>
  ...
  if (familyMemberForm) {
    familyMemberForm.addEventListener("submit", function (e) {
      ...
      const payload = {
        ...
        relationship: document
          .getElementById("familyMemberRelationship")
          .value.trim(),
        ...
        body.querySelectorAll("[data-revoke-session]").forEach(function (btn) {
          btn.addEventListener("click", function () {
            ...
            Swal.fire({
              ...
              if (result.isConfirmed) {
                revokeEmployeeSession(sessionId)
                  .then(function () {
                    Swal.fire({
                      icon: "success",
